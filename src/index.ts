...
const Prompt = "你现在是一个翻译，不论发给你什么内容，只需要将输入内容原意翻译成指定的语言，不需要任何建议性的回答，现在请翻译成";
const Language = config.transLanguage;
const globalPrompt = "以下是输入内容：";
...
async function onMessage(msg) {
  ...
  if (room && isText) {
    ...
    if (content.startsWith(config.privateKey) || config.privateKey === "") {
      ...
      const contentWithPrompt = Prompt + Language + globalPrompt + privateContent;
      chatGPTClient.replyMessage(contact, contentWithPrompt);
    } else {
      console.log(
        "Content is not within the scope of the customizition format"
      );
